...
* Se ha modificado la estructura de los comandos y se han implementado
...
struct _Command{
  T_Command cmd; /*!< Accion */
  char action[WORD_SIZE]; /*!< Nombre de la accion */
  char object[WORD_SIZE]; /*!< Nombre del objeto */
  char status[7]; /*!< Nombre del resultado */
};
...
STATUS command_set_status(Command* command, char* status){
  ...
  if(strcmp(status, "OK") != 0 && strcmp(status, "ERROR") && strcmp(status, " ") != 0) return ERROR;
